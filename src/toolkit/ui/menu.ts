 ...
 */
export function mainMenuKeyboard(columns = 2, labelFor?: (item: MainMenuItem) => string): InlineKeyboardMarkup {
  const cols = Math.max(1, Math.floor(columns));
  const items = mainMenuItems();
  const rows = [];
  for (let i = 0; i < items.length; i += cols) {
    rows.push(items.slice(i, i + cols).map((it) => inlineButton(labelFor?.(it) ?? it.label, it.data)));
  }
